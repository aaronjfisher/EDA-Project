...
###Initial Settings
nreps<-30 #number of plots
#how many clusters each rep has
clusters<-rep(3:8,length=nreps)[sample(nreps)]
N<-sample(50:80,nreps,replace=T) #Number in eachs sample
...
for(i in 1:nreps) {
	...
	for(a in 1:ci){
		for(b in 1:ci){
			if(a!=b){
				distab<-runif(1,1.5,5) #have to be some distance apart
				distmu[a,b]<-distab
				distmu[b,a]<-distab
			}
		}
...
getwd()
setwd("C:/Users/Aaron/Dropbox/EDA project")
save(list=c('clusters','nreps','refmats','samples','switchups'),file="data_for_clusts_network.RData")
